...
public class SimpleAirPlaneController : MonoBehaviour
{
    ...
    [SerializeField] InputAction yawright;
    [SerializeField] InputAction inputTurboRight;
    [SerializeField] InputAction inputTurboLeft;
    //Settings
    ...
    private void Start()
    {
        ...
        inputHActionTest = InputSystem.actions.FindAction("XRI RightHand Locomotion/fly");
        inputVActionTest = InputSystem.actions.FindAction("XRI LeftHand Locomotion/fly");
        yawleft = InputSystem.actions.FindAction("XRI LeftHand Locomotion/yawleft");
        yawright = InputSystem.actions.FindAction("XRI RightHand Locomotion/yawright");
        inputTurboRight = InputSystem.actions.FindAction("XRI RightHand Locomotion/turboright");
        inputTurboLeft = InputSystem.actions.FindAction("XRI LeftHand Locomotion/turboleft");

        if (inputHActionTest != null)
        {
            Debug.Log("inputHActionTest found");
    ...
    private void HandleInputs()
    {
        ...
        // handle various inputs types.

        //If plane is dead do not handle inputs
        //Rotate inputs
        ...
        if (VRInput)
        {
            Vector2 moveValue = inputVActionTest.ReadValue<Vector2>();
            Vector2 doubleMoveValue = inputHActionTest.ReadValue<Vector2>();


            // inputH;
           // Debug.Log("inputV value is:  x " +  moveValue.x + " : y " + moveValue.y);

            inputH = moveValue.x;
            inputV = moveValue.y;

            inputH = doubleMoveValue.x;
            inputV = doubleMoveValue.y;

            inputYawLeft = yawleft.IsPressed() ? true : false;
            inputYawRight = yawright.IsPressed() ? true : false;
